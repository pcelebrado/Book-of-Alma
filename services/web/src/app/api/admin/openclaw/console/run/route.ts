...
export async function POST(request: NextRequest) {
  ...
  const body = (await parseJsonBody<ConsoleRequestBody>(request)) ?? {};

  try {
    const payload = await coreFetch<ConsoleResponse, ConsoleRequestBody>(
      '/internal/openclaw/console/run',
      {
        method: 'POST',
        body,
        uid: session.id,
    ...
    return apiError('internal_error', 'Unable to run OpenClaw console command', 500);
  }
}
