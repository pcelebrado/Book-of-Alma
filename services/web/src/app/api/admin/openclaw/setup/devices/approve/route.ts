...
export async function POST(request: NextRequest) {
  ...
  const body = (await parseJsonBody<DeviceApproveBody>(request)) ?? {};

  try {
    const payload = await coreFetch('/internal/openclaw/setup/devices/approve', {
      method: 'POST',
      body,
      uid: session.id,
      role: session.role,
    ...
    return apiError('internal_error', 'Unable to approve device', 500);
  }
}
